...
def change_key(og_command):
    ...
    if piece_name not in pieces_and_info:
        ...
        composer = list(pieces_and_info[piece_name].keys())
        pieces_and_info[piece_name][composer[0]] = new_key
...
for _ in range(number_of_pieces):
    ...
    piece_information = input().split("|")
    piece = piece_information[0]
    composer = piece_information[1]
    key = piece_information[2]
...
for each_piece in arranged_pieces:
    piece = each_piece[0]
    for composer, key in each_piece[1].items():
